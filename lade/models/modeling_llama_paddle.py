...
def j_expand_2d_mask(mask, dtype, tgt_length):
    """
    Expands attention_mask from `[batch_size, src_length]` to `[batch_size, 1, tgt_length, src_length]` for LADE
    """
    batch_size, src_length = mask.shape[0], mask.shape[-1]
    tgt_length = tgt_length if tgt_length is not None else src_length
    ...
    mask = mask[:, None, None, :]
    mask.stop_gradient = True
    expanded_mask = mask.expand([batch_size, 1, tgt_length, src_length]).astype(dtype)
    
    inverted_mask = 1.0 - expanded_mask
    assert paddle.finfo(dtype).min != -float("Inf") and paddle.finfo(dtype).min != float("Inf"), f"min of {dtype} is {paddle.finfo(dtype).min}"
    return paddle.where(inverted_mask.astype(paddle.bool), paddle.finfo(dtype).min, inverted_mask)
...
def j_make_causal_mask_multilevel(
    ...
    mask = paddle.to_tensor(mask, place=place)
    assert not paddle.isinf(mask).any(), f"mask has inf at start,dtype: {dtype}, {paddle.finfo(dtype).min}\n{mask}"
    
    if is_prefill:
        mask_cond = paddle.arange(mask.shape[-1])
        mask_cond = paddle.to_tensor(mask_cond, place=place)
        mask.masked_fill_(mask_cond < (mask_cond + 1).unsqueeze(1), 0)
        mask = mask.to(dtype)
        assert past_key_values_length == 0
        assert guess is None
        return paddle.expand(mask[None, None, :, :], [bsz, 1, tgt_len, tgt_len + past_key_values_length])
    
    tiny_mask_size = level_sizes[-1] # + 1
    mask_cond = paddle.arange(tiny_mask_size)
    mask_cond = paddle.to_tensor(mask_cond, place=place)
    hm = mask_cond < (mask_cond + 1).view([tiny_mask_size, 1])
    # Offset when guess multiple tokens and not copy kv-cache 
    level_offset = tgt_len - (sum(level_sizes) + 1) - (len(guess) if guess is not None else 0)
    # Offset for distributed inference 
    dist_offset = (1 + level_sizes[0] - level_sizes[-1])
    assert dist_offset >= 0 and level_offset >= 0
    assert not paddle.isinf(mask).any(), "mask has inf before branch"
    
    if guess is not None:
        lguess = len(guess)
        dtype_min = paddle.finfo(dtype).min
        if guess_size == 2:
            small_m = paddle.tile(paddle.to_tensor([0, dtype_min], dtype=dtype), lguess // 2)[:-1]
            mask[-lguess:, -lguess:] = mask[-lguess:, -lguess:].fill_diagonal_(0).diagonal_scatter(small_m, -1)
        elif guess_size == 3:
            small_m1 = paddle.tile(paddle.to_tensor([0, 0, dtype_min], dtype=dtype), lguess // 3)[:-1]
            small_m2 = paddle.tile(paddle.to_tensor([0, dtype_min, dtype_min], dtype=dtype), lguess // 3)[:-2]
            mask[-lguess:, -lguess:] = mask[-lguess:, -lguess:].fill_diagonal_(0).diagonal_scatter(small_m1, -1).diagonal_scatter(small_m2, -2)
        elif guess_size == 4:
            small_m1 = paddle.tile(paddle.to_tensor([0, 0, 0, dtype_min], dtype=dtype), lguess // 4)[:-1]
            small_m2 = paddle.tile(paddle.to_tensor([0, 0, dtype_min, dtype_min], dtype=dtype), lguess // 4)[:-2]
            small_m3 = paddle.tile(paddle.to_tensor([0, dtype_min, dtype_min, dtype_min], dtype=dtype), lguess // 4)[:-3]
            ...
            small_m1 = paddle.tile(paddle.to_tensor([0, 0, 0, 0, dtype_min], dtype=dtype), lguess // 5)[:-1]
            small_m2 = paddle.tile(paddle.to_tensor([0, 0, 0, dtype_min, dtype_min], dtype=dtype), lguess // 5)[:-2]
            small_m3 = paddle.tile(paddle.to_tensor([0, 0, dtype_min, dtype_min, dtype_min], dtype=dtype), lguess // 5)[:-3]
            small_m4 = paddle.tile(paddle.to_tensor([0, dtype_min, dtype_min, dtype_min, dtype_min], dtype=dtype), lguess // 5)[:-4]
            ...
            small_m1 = paddle.tile(paddle.to_tensor([0, 0, 0, 0, 0, dtype_min], dtype=dtype), lguess // 6)[:-1]
            small_m2 = paddle.tile(paddle.to_tensor([0, 0, 0, 0, dtype_min, dtype_min], dtype=dtype), lguess // 6)[:-2]
            small_m3 = paddle.tile(paddle.to_tensor([0, 0, 0, dtype_min, dtype_min, dtype_min], dtype=dtype), lguess // 6)[:-3]
            small_m4 = paddle.tile(paddle.to_tensor([0, 0, dtype_min, dtype_min, dtype_min, dtype_min], dtype=dtype), lguess // 6)[:-4]
            small_m5 = paddle.tile(paddle.to_tensor([0, dtype_min, dtype_min, dtype_min, dtype_min, dtype_min], dtype=dtype), lguess // 6)[:-5]
            ...
            small_m1 = paddle.tile(paddle.to_tensor([0, 0, 0, 0, 0, 0, dtype_min], dtype=dtype), lguess // 7)[:-1]
            small_m2 = paddle.tile(paddle.to_tensor([0, 0, 0, 0, 0, dtype_min, dtype_min], dtype=dtype), lguess // 7)[:-2]
            small_m3 = paddle.tile(paddle.to_tensor([0, 0, 0, 0, dtype_min, dtype_min, dtype_min], dtype=dtype), lguess // 7)[:-3]
            small_m4 = paddle.tile(paddle.to_tensor([0, 0, 0, dtype_min, dtype_min, dtype_min, dtype_min], dtype=dtype), lguess // 7)[:-4]
            small_m5 = paddle.tile(paddle.to_tensor([0, 0, dtype_min, dtype_min, dtype_min, dtype_min, dtype_min], dtype=dtype), lguess // 7)[:-5]
            small_m6 = paddle.tile(paddle.to_tensor([0, dtype_min, dtype_min, dtype_min, dtype_min, dtype_min, dtype_min], dtype=dtype), lguess // 7)[:-6]
            ...
            for i in range(guess_size - 1): # 7 : 0 - 5
                small_l = [0] * (guess_size - i - 1)  + [dtype_min] * (i + 1)
                small_m = paddle.tile(paddle.to_tensor(small_l), lguess // guess_size)[:-1 - i]
                mask[-lguess:, -lguess:] = mask[-lguess:, -lguess:].diagonal_scatter(small_m, -1 - i)
        mask[-lguess:, :level_offset + 1] = 0   
    else:
        lguess = 0
    # assert not paddle.isinf(mask).any(), f"mask has inf\n{mask}"

    all_offset = level_offset + dist_offset
    offset = all_offset + tiny_mask_size
    if all_offset > 0:
        mask_offset = paddle.arange(all_offset, place=place)
        ...
        mask[all_offset : mask.size(0)-lguess, :all_offset] = 0
        # assert tgt_len == sum(level_sizes) + 1 , f"tgt levels not equal {tgt_len} {level_sizes} {la_mask_offset}"
    
    # assert not paddle.isinf(mask).any(), "mask has inf before range(len(level_sizes))"
    for l in range(len(level_sizes)):
        if l > 0:
            assert level_sizes[l] == tiny_mask_size
        mask[offset * l : offset * (l + 1), all_offset : offset].masked_fill_(hm, 0)
        for row in range(1, l + 1):
            mask[offset * l : offset * (l + 1), offset * row : offset * (row + 1)].fill_diagonal_(0)

    if past_key_values_length > 0 or la_mask_offset > 0:
        mask = paddle.concat(
            [paddle.zeros([tgt_len, past_key_values_length + la_mask_offset], dtype=dtype), mask]
            , axis=-1)
...
class LlamaAttention(nn.Layer):
    ...
    def forward(
        ...
        if output_attentions:
            ...
            attn_output = outputs
        # if paddle.any(paddle.isinf(attn_output)):
            # print(f"attn_output, {attn_output.shape}, {paddle.max(attn_output).item()}, {paddle.min(attn_output).item()}")
            # exit()
...
class LlamaDecoderLayer(nn.Layer):
    ...
    def forward(
        ...
        # if paddle.any(paddle.isinf(hidden_states)):
            # print(f"self_attn, {hidden_states.shape}, {paddle.max(hidden_states).item()}, {paddle.min(hidden_states).item()}")
            # exit()
        ...
        residual = hidden_states
        # if paddle.any(paddle.isinf(hidden_states)):
            # print(f"residual>0, {hidden_states.shape}, {paddle.max(hidden_states).item()}, {paddle.min(hidden_states).item()}")
            # exit()
        hidden_states = self.post_attention_layernorm(hidden_states)
        # if paddle.any(paddle.isinf(hidden_states)):
            # print(f"attn>0, {hidden_states.shape}, {paddle.max(hidden_states).item()}, {paddle.min(hidden_states).item()}")
            # exit()
        hidden_states = self.mlp(hidden_states)
        # if paddle.any(paddle.isinf(hidden_states)):
            # print(f"mlp>0, {hidden_states.shape}, {paddle.max(hidden_states).item()}, {paddle.min(hidden_states).item()}")
            # exit()
        hidden_states = residual + hidden_states
        # if paddle.any(paddle.isinf(hidden_states)):
            # print(f"decoder>0, {hidden_states.shape}, {paddle.max(hidden_states).item()}, {paddle.min(hidden_states).item()}")
            # exit()
...
class LlamaPretrainedModel(PretrainedModel):
    ...
    #     for layer_index in range(config.num_hidden_layers):
    #         layer_mappings = [
    #             [f"layers.{layer_index}.attention.wq.weight", f"layers.{layer_index}.self_attn.q_proj.weight", "transpose"],
    #             [f"layers.{layer_index}.attention.wk.weight", f"layers.{layer_index}.self_attn.k_proj.weight", "transpose"],
    #             [f"layers.{layer_index}.attention.wv.weight", f"layers.{layer_index}.self_attn.v_proj.weight", "transpose"],
    #             [f"layers.{layer_index}.attention.wo.weight", f"layers.{layer_index}.self_attn.o_proj.weight", "transpose"],
    #             [f"layers.{layer_index}.self_attn.rotary_emb.inv_freq"],
    #             [f"layers.{layer_index}.feed_forward.w1.weight", f"layers.{layer_index}.mlp.gate_proj.weight", "transpose"],
    #             [f"layers.{layer_index}.feed_forward.w2.weight", f"layers.{layer_index}.mlp.down_proj.weight", "transpose"],
    #             [f"layers.{layer_index}.feed_forward.w3.weight", f"layers.{layer_index}.mlp.up_proj.weight", "transpose"],
    #             [f"layers.{layer_index}.attention_norm.weight", f"layers.{layer_index}.input_layernorm.weight"],
    #             [f"layers.{layer_index}.ffn_norm.weight", f"layers.{layer_index}.post_attention_layernorm.weight"],
    #         ]
    #         model_mappings.extend(layer_mappings)
    ...
    #             # mapping[0] = "model." + mapping[0]
    #             mapping[1] = "llama." + mapping[1]
    #         model_mappings.append(["output.weight", "lm_head.weight", "transpose"])
...
class LlamaModel(LlamaPretrainedModel):
    ...
    # For LADE
    @staticmethod
    def j_prepare_decoder_attention_mask(attention_mask, input_shape, inputs_embeds, past_key_values_length, *args):
        # Create causal mask for lookahead
        # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
        WINDOW_SIZE, is_prefill, guess, guess_size, not_seq, continue_all, la_mask_offset, level_sizes = args
        ...
        if input_shape[-1] > 1:
            combined_attention_mask = j_make_causal_mask_multilevel(
                ...
                place=inputs_embeds.place,
                past_key_values_length=past_key_values_length,
            )

        if attention_mask is not None:
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            expanded_attn_mask = j_expand_2d_mask(attention_mask, inputs_embeds.dtype, tgt_length=input_shape[-1])
    ...
    def LlamaModeljforward(
        ...
        **kwargs,
        ...
        # `forward` method for lookahead decoding
        if self.sequence_parallel and use_cache:
            raise ValueError("We currently only support sequence parallel without cache.")
        ...
        if position_ids is None:
            ...
            position_ids = paddle.reshape(position_ids, shape=[-1, seq_length])
            position_ids = position_ids.astype('int64')
        
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_ids)

        if attention_mask is None:  # [bs, seq_len]
            attention_mask = paddle.ones((batch_size, seq_length_with_past), dtype=paddle.bool)
        ...
        if use_flash:
            ...
            attention_mask = self.j_prepare_decoder_attention_mask(
                ...
                WINDOWS_SIZE, is_prefill, guess, guess_size, not_seq, continue_all, la_mask_offset, level_sizes
            )

        level_offset = seq_length - (sum(level_sizes) + 1) - (len(guess) if guess is not None else 0)   # input.len - sum(len(2D window)) - 1 - len(guess)
        dist_offset = (1 + level_sizes[0] - level_sizes[-1])

        lookahead = [level_sizes[-1], len(level_sizes) + 1, len(guess) // guess_size if guess is not None else 0, 
        0, level_offset + dist_offset, level_offset, 0] # Only used with flash attention
...
class LlamaForCausalLM(LlamaPretrainedModel):
    ...
    def jforward_multilevel(
        ...
        if past_key_values is not None:
            past_size = past_key_values[0][0].shape[1]
        else:
            past_size = 0
        ...
        lst_id = position_ids[0][-1].item() # Last token's position
        level_sizes = []                    # Length of each level in 2D window
        all_past = []                       # All past tokens from `past_tokens`
        ids_list = []                       # Positions of all past tokens
        attn_size = 0                       # Number of all past tokens

        if swap_axis_for_flash:
            pass
        else:
            for l in range(fill_level + 1):
                all_past += past_tokens[l]
                attn_size += len(past_tokens[l])
                level_sizes.append(len(past_tokens[l]))

                if l == 0:
                    ids_list += list(range(lst_id + 1, lst_id + 1 + len(past_tokens[l])))
                else:
                    offset = len(past_tokens[0]) + 1 - len(past_tokens[l])
                    ids_list += list(range(lst_id + l + offset, lst_id + l + offset + len(past_tokens[l])))
        
        # Update model_inputs
        if guess_tokens is not None:
            input_ids = paddle.concat(
                [input_ids, paddle.to_tensor(all_past + guess_tokens, dtype=input_ids.dtype, place=input_ids.place).unsqueeze(0)]
                , axis=1)
            guess_ids = list(range(lst_id + 1, lst_id + 1 + guess_size)) * (len(guess_tokens) // guess_size)    # guess_size is level-1
            position_ids = paddle.concat(
                [position_ids, paddle.to_tensor(ids_list + guess_ids, dtype=input_ids.dtype, place=input_ids.place).unsqueeze(0)]
                , axis=1)
            attention_mask = paddle.concat(
                [attention_mask, paddle.ones([1, attn_size + len(guess_tokens)], dtype=input_ids.dtype)]
                , axis=1)
        else:
            input_ids = paddle.concat(
                [input_ids, paddle.to_tensor(all_past, dtype=input_ids.dtype, place=input_ids.place).unsqueeze(0)]
                , axis=1)
            position_ids = paddle.concat(
                [position_ids, paddle.to_tensor(ids_list, dtype=input_ids.dtype, place=input_ids.place).unsqueeze(0)]
                , axis=1)
            attention_mask = paddle.concat(
                [attention_mask, paddle.ones([1, attn_size], dtype=input_ids.dtype)]
                , axis=1)
        
        step_len = attention_mask.shape[1]  # Lookahead steps

        outputs = self.llama.LlamaModeljforward(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_values=past_key_values,
        ...
        ret = CausalLMOutputWithCrossAttentions(
            loss=loss,
            logits=logits,  # place=input_ids.place
            past_key_values=outputs.past_key_values,
            hidden_states=outputs.hidden_states,
            attentions=outputs.attentions,
        )

        ret.kvcache_len = prefill_size + past_size  # input_ids.len + cache.len
        ret.step_len = step_len # Lookahead steps

        lguess = 0
        if guess_tokens is not None:
            lguess = len(guess_tokens)  # Length of guess tokens, should be k * (N-1)

        out_logits = ret.logits[:, prefill_size - 1, :] # Logits of decoding
        ret.out_logits = paddle.to_tensor(out_logits, place=input_ids.place)
        
        assert fill_level != -1
        window = len(past_tokens[fill_level])           # Last W tokens of 2D window
        start = ret.logits.shape[1] - window - lguess   # Start of the last W tokens
        end = ret.logits.shape[1] - lguess              # End of the last W tokens
        
        if swap_axis_for_flash:
            pass        
        else:
            inp_logits = ret.logits[:, start:end, :]    # Logits of Lookahead branch
            ret.inp_logits = paddle.to_tensor(inp_logits, place=input_ids.place)
        
        if lguess > 0:
            guess_logits = ret.logits[:, -lguess:, :]   # Logits of verification branch
            ret.guess_logits = paddle.to_tensor(guess_logits, place=input_ids.place)
